...
As mentioned in section \ref{etw}, whenever a provider wants to register itself against a particular session, it has to call the function {\bfseries EtwRegister}.
...
\end{itemize}
  ...
  \subsubsection{\bfseries{Understanding the layout of the function}}
  ...
  \subsubsection{\bfseries{Check if GUID for this provider already exists}}
...
\subsubsection{\bfseries{If not found, create a new one}}
...
Once $EtwpAllocGuidEntry$ is executed, the pointer to heap memory holding the $\_ETW\_GUID\_ENTRY$ structure is returned. The next step is insert this entry into $EtwpGuidHashTable$. To perform that action, first it look for the correct place to insert it as depicted previously.
...
\subsubsection{\bfseries{Return the handler}}
...
No matter if the provider's GUID existed previously or not, at this point it exist a pointer to a $\_ETW\_GUID\_ENTRY$ structure holding its data and already inside the main structures of ETW. 
